...
export default function Parvus (userOptions) {
  ...
  const pinchZoom = (currentImg) => {
    ...
    if (!isPinching || IS_NEW_POINTER_COMBINATION) {
      ...
      if ((!currentImg.style.transformOrigin && currentScale === 1) ||
        (currentScale === 1 && IS_NEW_POINTER_COMBINATION)) {
  ...
  const bindEvents = () => {
    ...
    // Pointer events
    lightbox.addEventListener('pointerdown', pointerdownHandler, { passive: false })
    lightbox.addEventListener('pointerup', pointerupHandler, { passive: true })
    lightbox.addEventListener('pointermove', pointermoveHandler, { passive: false })
  }
